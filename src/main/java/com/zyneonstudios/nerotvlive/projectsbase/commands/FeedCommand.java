    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        if(s.hasPermission("zyneon.team")) {
            if(s instanceof Player p) {
                if(args.length==0) {
                    p.setFoodLevel(20);
                    Communicator.sendInfo(p,"Du hast dich gefüttert§8.");
                } else {
                    if(Bukkit.getPlayer(args[0])!=null) {
                        Player t = Bukkit.getPlayer(args[0]);
                        t.setFoodLevel(20);
                        Communicator.sendInfo(p,"Du hast den*die Spieler*in §e"+t.getName()+"§7 gefüttert§8.");
                    } else {
            ...
            if(args.length==0) {
                ...
                if(Bukkit.getPlayer(args[0])!=null) {
                    Player t = Bukkit.getPlayer(args[0]);
                    t.setFoodLevel(20);
                    Communicator.sendInfo(s,"Du hast den*die Spieler*in §e"+t.getName()+"§7 gefüttert§8.");
                } else {
